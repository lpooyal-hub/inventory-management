# Inventory Management MVP
 ...
 지인 요청으로 복잡한 ERP 대신, 매일 출고 기록과 월별 파일 업로드를 함께 쓰는 재고관리 MVP입니다.
...
- `최근 3개월 월 평균 출고량 = 기준월을 제외한 직전 3개월 OUT 합계 / 3`
- `남은 예상 출고량 = 최근 3개월 월 평균 출고량 - 이번 달 출고량`
- `평균 대비 재고수량 = 현재고 - 최근 3개월 월 평균 출고량`
- `평균 대비 재고수량 < 0` 이면 `부족`
- `평균 대비 재고수량 <= -(현재고 * 0.3)` 이면 `매우 부족`

예시:

- 기준월이 `2026년 7월`이면 최근 3개월 평균은 `2026년 4월 ~ 6월` 기준입니다.
- 기준월은 서버 위치가 아니라 `Asia/Seoul`(한국시간) 기준으로 계산합니다.
...
- 재고현황의 기준월은 한국시간 기준 현재 월이며, 화면 상단에 기준월과 3개월 평균 산정 기간을 함께 표시합니다.
- `남은 예상 출고량`은 참고용 지표이고, 부족 판단 자체는 `현재고 - 최근 3개월 평균 출고량`으로 계산합니다.
